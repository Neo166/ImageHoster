...
public class ImageController {
    ...
    public String editImage(@RequestParam("imageId") Integer imageId, Model model, HttpSession session, RedirectAttributes redirectAttr) {
        //Solving the PartA Issue2: Restricting edit option to only Image owners. Adding check for userID.
    ...
    public String deleteImageSubmit(@RequestParam(name = "imageId") Integer imageId, Model model, HttpSession session) {
        //Solving the PartA Issue2: Restricting delete option to only Image owners. Adding check for userID.
        Image image = imageService.getImage(imageId);
        User user = (User) session.getAttribute("loggeduser");
